...
namespace Valkyrie.Services
{
    public class GitHubService : IGitHubService
    {
        ...
        public GitHubService(HttpClient httpClient, IConfiguration configuration)
        {
            ...
            _httpClient.DefaultRequestHeaders.UserAgent.ParseAdd("Valkyrie-App");
        ...
        public async Task<(List<string>? Files, List<DependencyPackage>? Packages, List<Ecosystem>? DetectedEcosystems, string? ErrorMessage)> GetRepositoryFilesAsync(string repositoryUrl)
        {
            ...
            try
            {
                ...
                if (files.Count > 5000)
                {
                    return (null, null, null, "This repository is too large to scan in full. Valkyrie currently supports repositories up to 5000 files.");
                }
                ...
                {
                    return (null, null, null, "No supported dependency files found. Valkyrie supports npm (package.json), NuGet (*.csproj), Python (requirements.txt), Java (pom.xml), Go (go.mod), Ruby (Gemfile.lock), and PHP (composer.json).");
                }
